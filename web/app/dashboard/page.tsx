...
export default function AppDashboard() {
    ...
    useEffect(() => {
        const init = async () => {
            // Skip auth check if running in Electron (desktop app)
            const api = getElectronAPI();
            if (api) {
                console.log('[Dashboard] Running in Electron, skipping Supabase auth');
                setUserEmail('Desktop User');
                setCredits(0);
                return;
            }

            // Web app: require Supabase authentication

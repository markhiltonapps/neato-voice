...
const store = new Store({
    defaults: {
        ...
        enhanceText: false // Professional text enhancement (elevates language, adds impact)
    }
});

// Initialize version history storage for rollback feature
const versionStore = new Store({
    name: 'version-history',
    defaults: {
        versions: [] // Array of { version, installedDate }
    }
});

// Track last transcription for F2 enhancement
let lastRawTranscription = null;
let lastPastedText = null;
let lastPasteTimestamp = null;
logger('[State] Transcription tracking initialized');
...
// Track version for rollback feature
function trackVersion() {
    const currentVersion = app.getVersion();
    const history = versionStore.get('versions', []);

    // Check if this version is already tracked
    const exists = history.find(v => v.version === currentVersion);
    if (!exists) {
        history.unshift({
        ...
        versionStore.set('versions', history.slice(0, 3));
        logger(`[VersionTracker] Tracked version ${currentVersion}`);
    }
}

// Perform rollback to a previous version
async function performRollback(targetVersion) {
    try {
        logger(`[Rollback] Starting rollback to v${targetVersion}`);

        // Download installer for target version from GitHub
        const downloadUrl = `https://github.com/markhiltonapps/neato-voice/releases/download/v${targetVersion}/Neato.Voice.Setup.${targetVersion}.exe`;

        logger(`[Rollback] Downloading from ${downloadUrl}`);

        // Download to temp directory
        const tempPath = path.join(app.getPath('temp'), `Neato-Voice-Rollback-${targetVersion}.exe`);
        ...
        logger(`[Rollback] Downloaded to ${tempPath}`);

        // Execute installer and quit
        const { exec } = require('child_process');
        exec(`"${tempPath}"`, (error) => {
            if (error) logger(`[Rollback] Error launching installer: ${error}`);
        });

        logger('[Rollback] Installer launched, quitting app...');

        // Quit after short delay to allow installer to start
        setTimeout(() => {
            app.isQuitting = true;
            app.quit();
        }, 1000);
        ...
        logger(`[Rollback] Error: ${error.message}`);
        return { success: false, error: error.message };
    }
}

// Handle text enhancement - reads clipboard directly
logger('[Main] Registering enhance-text IPC handler...');
ipcMain.handle('enhance-text', async (event) => {
    try {
        // Read clipboard directly (no focus needed!)
        const text = clipboard.readText();
        logger(`[Enhancement] Reading clipboard, length: ${text.length} chars`);

        if (!text || text.trim().length === 0) {
            logger('[Enhancement] Clipboard is empty');
            return { success: false, error: 'Clipboard is empty - copy text first with Ctrl+C' };
        }

        logger(`[Enhancement] Enhancing ${text.length} characters from clipboard...`);

        // Get API key with fallback for packaged app
        let apiKey = process.env.ANTHROPIC_API_KEY;

        // Fallback for packaged app (Base64 encoded to avoid secret scanning)
        // Fallback removed for security
        if (!apiKey && app.isPackaged) {
            logger('[Enhancement] No API key found in env, and fallback layout removed.');
        }

        if (!apiKey) {
            logger('[Enhancement] ERROR: No ANTHROPIC_API_KEY found');
            return { success: false, error: 'Anthropic API key not configured' };
        }

        logger('[Enhancement] API key found, calling Claude...');

        const anthropic = new Anthropic({ apiKey });

        const systemPrompt = `You are an AI assistant designed to refine and complete text.
Your goal is to take a user's partial or raw text input and provide a polished, complete, and contextually appropriate continuation or refinement.

RULES:
1. COMPLETE THOUGHTS: If the input seems like a partial sentence or idea, complete it naturally.
2. GRAMMAR AND SPELLING: Correct any grammatical errors or typos.
3. CLARITY AND CONCISENESS: Improve sentence structure for better readability and remove unnecessary words.
4. PROFESSIONAL TONE: Maintain a professional and neutral tone unless otherwise implied by the context.
5. PRESERVE MEANING: Do not alter the core meaning or intent of the original text.
6. NO INTRODUCTIONS/CONCLUSIONS: Provide only the refined or completed text, without conversational intros or outros.
7. NO QUOTATION MARKS: Do NOT wrap the output in quotation marks or any other delimiters. Return the raw text directly.

Example:
User Input: "The project was a big success, we hit all our goals and the team worked hard."
Your Output: The project was a significant success; we achieved all our objectives, and the team demonstrated exceptional dedication.

User Input: "I need to send an email to the client about the new"
Your Output: I need to send an email to the client about the new proposal.

User Input: "Let's discuss this further in our next meeting"
Your Output: Let's discuss this further in our next meeting.

OUTPUT: Refined or completed text ONLY.`;
        ...
        const response = await anthropic.messages.create({
            ...
            messages: [{
                role: "user",
                content: `Refine and complete this text:\n\n"${text}"`
            }]
        });

        const refinedText = response.content[0].text.trim();
        lastPastedText = refinedText; // Store for potential re-selection
        ...
        { type: 'separator' },
        {
            ...
            click: async () => {
                ...
                const result = await dialog.showMessageBox({
                    ...
                    message: `Rollback to v${previousVersion.version}?`,
                    detail: `This will replace your current version with ${previousVersion.version}. The previous installer will be downloaded from GitHub (~82 MB). You can update again later.`,
                ...
                if (result.response === 0) {
                    logger(`[Rollback] User initiated rollback to v${previousVersion.version}`);
                    // Start rollback process
                    const rollbackResult = await performRollback(previousVersion.version);
                    if (!rollbackResult.success) {
                        dialog.showErrorBox('Rollback Failed', rollbackResult.error || 'Failed to rollback version');
                    }
                }
            }
        },
...
function registerHotkeys() {
    try {
        // Simple, mnemonic hotkey scheme using Alt+Letter keys
        // Avoids Fn key conflicts on laptops where F-keys require Fn+F1
        logger(`[Hotkeys] Registering global hotkeys...`);

        // Alt+R: Toggle Recording
        const ctrlRRegistered = globalShortcut.register('CommandOrControl+R', () => {
            logger('[Hotkey] Ctrl+R pressed - Toggle Recording');
        ...
        if (!ctrlRRegistered) {
            logger(`[Hotkeys] WARNING: Failed to register Ctrl+R hotkey`);
        } else {
            logger(`[Hotkeys] ✓ Ctrl+R registered: Toggle Recording`);
        }

        // Ctrl+E: Enhance Selected Text (with auto-copy)
        const ctrlERegistered = globalShortcut.register('CommandOrControl+E', () => {
            logger('[Hotkey] Ctrl+E pressed - Enhance Selected Text');
            if (mainWindow) {
                // User copies manually with Ctrl+C, then presses Ctrl+E
                logger('[Hotkey] Sending enhance event to renderer');
                mainWindow.webContents.send('trigger-enhance-selected');
            }
        });

        if (!ctrlERegistered) {
            logger('[Hotkey] ERROR: Failed to register Ctrl+E');
        } else {
            logger('[Hotkey] Successfully registered: Ctrl+E');
        }

        logger(`[Hotkeys] Registration complete. Alt+R=${ctrlRRegistered}, Ctrl+E=${ctrlERegistered}`);
    } catch (e) {
...
function setupAutoUpdater() {
    ...
    autoUpdater.on('update-available', (updateInfo) => {
        logger(`[Updater] Update available: v${updateInfo.version}`);

        // Extract release notes from updateInfo
        let releaseNotes = updateInfo.releaseNotes || 'No release notes available.';

        // Handle different releaseNotes formats (string or array)
        let releaseNotesText = '';
        if (typeof releaseNotes === 'string') {
            releaseNotesText = releaseNotes;
        } else if (Array.isArray(releaseNotes)) {
            releaseNotesText = releaseNotes.map(note => note.note || '').join('\n');
        }

        // Format and truncate release notes for display (max 500 chars)
        const formattedNotes = releaseNotesText
            .replace(/#{1,6}\s/g, '') // Remove markdown headers
            .replace(/[*_]/g, '') // Remove markdown emphasis
            .substring(0, 500);

        // Show changelog dialog to user
        dialog.showMessageBox({
            type: 'info',
            title: 'Update Available',
            message: `Version ${updateInfo.version} is available!`,
            detail: `What's New:\n\n${formattedNotes}${releaseNotesText.length > 500 ? '...' : ''}`,
            buttons: ['Download Now', 'Later'],
            defaultId: 0,
            cancelId: 1
        }).then(({ response }) => {
            if (response === 0) {
                logger('[Updater] User accepted update - download will begin automatically');
            } else {
                logger('[Updater] User postponed update');
            }
        });
    });
...
if (!gotTheLock) {
    ...
    app.whenReady().then(() => {
        ...
        registerHotkeys();
        setupAutoUpdater();
        trackVersion(); // Track current version for rollback
...
ipcMain.handle('get-deepgram-key', () => {
    logger('[Deepgram] IPC handler called: get-deepgram-key');
    logger(`[Deepgram] App is packaged: ${app.isPackaged}`);
    logger(`[Deepgram] Checking environment variables...`);

    let key = process.env.DEEPGRAM_API_KEY || process.env.NEXT_PUBLIC_DEEPGRAM_API_KEY;

    // Fallback for packaged app where .env is not included
    if (!key && app.isPackaged) {
        logger('[Deepgram] .env not found in packaged app, using hardcoded key');
        key = '5bf3f9ffc2a9002ff57f73db7bc17d34ce1796a3';
    }

    if (!key) {
        logger('[Deepgram] ERROR: No DEEPGRAM_API_KEY found in environment');
        logger(`[Deepgram] DEEPGRAM_API_KEY: ${process.env.DEEPGRAM_API_KEY ? 'EXISTS' : 'MISSING'}`);
        logger(`[Deepgram] NEXT_PUBLIC_DEEPGRAM_API_KEY: ${process.env.NEXT_PUBLIC_DEEPGRAM_API_KEY ? 'EXISTS' : 'MISSING'}`);
        logger(`[Deepgram] Current env path attempted: ${envPath}`);
        return null;
    }

    logger(`[Deepgram] API key found! Length: ${key.length} characters`);
    logger('[Deepgram] Returning key to renderer process');
    return key;
});
...
ipcMain.handle('refine-text', async (event, text, options = {}) => {
    try {
        ...
        // Store raw transcription for F2 enhancement
        lastRawTranscription = text;
        lastPasteTimestamp = Date.now();
        logger('[Refinement] Stored transcription for potential F2 enhancement');
        ...
        if (isTranslation) {
            ...
            logger(`[Refinement] Mode: STANDARD DICTATION (basic bulletization only)`);
        }
        ...
        let systemPrompt = `You are a STRICT list formatting engine for Neato Voice dictation software.
...
If there's clearly only ONE action/thing, just clean it up (remove filler words, fix grammar).`;
        ...
        let userPrompt = `STUDY THESE EXAMPLES. Your output MUST match this EXACT pattern:
...
OUTPUT:`;
        ...
        // Enhancement logic removed - now handled by separate F2 hotkey

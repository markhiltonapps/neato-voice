...
ipcMain.on('recording-state-change', (event, state) => {
    // state can be 'recording', 'processing', 'error', 'idle' (or boolean true/false for legacy)
    const isRecording = state === true || state === 'recording';
    const isProcessing = state === 'processing';
    const isError = state === 'error'; // Keep overlay visible on error

    if (overlayWindow && !overlayWindow.isDestroyed()) {
        if (isRecording || isProcessing || isError) {
            overlayWindow.setOpacity(1.0);
            overlayWindow.showInactive(); // Show overlay
            // Ensure it's always on top
            overlayWindow.setAlwaysOnTop(true, 'screen-saver');
        } else {
            // Only hide if strictly 'idle' or false
            overlayWindow.hide();
        }
    }

    // Manage Main Window visibility state
    // Restoring main window opacity if we are done recording
...
4. **CRITICAL:** Detect parallel items, lists, or steps. YOU MUST format them as a Markdown bulleted list.
   - Example Input: "I need to buy eggs, wash the car, and walk the dog."
   - Example Output:
     * Buy eggs
     * Wash the car
     * Walk the dog
5. Output ONLY the refined text. Do not add "Here is the text" or any conversational filler.`;
...
3. Output ONLY the translated text, no preamble or explanations.`;
        ...
        throw error; // Throwing error allows the renderer to catch it and show the red box
    }
});
